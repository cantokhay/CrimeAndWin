...
{
    public class RevertPlayerStatsCommandConsumer : IConsumer<RevertPlayerStatsCommand>
    {
        private readonly IWriteRepository<Player> _playerWrite;
        private readonly IReadRepository<Player> _playerRead;

        public RevertPlayerStatsCommandConsumer(IWriteRepository<Player> playerWrite, IReadRepository<Player> playerRead)
        {
            _playerWrite = playerWrite;
            _playerRead = playerRead;
        }

        public async Task Consume(ConsumeContext<RevertPlayerStatsCommand> context)
        {
            ...
            try
            {
                ...
                await context.Publish(new PlayerStatsUpdatedEvent 
                {
                    ...
                    IsSuccess = saved > 0,
                    FailReason = saved > 0 ? null : "Could not save reverted player stats."
                });
            }
            catch (Exception ex)
            {
