    ...
    public sealed class LoginCommandHandler : IRequestHandler<LoginCommand, ResultAuthDTO>
    {
        ...
        public async Task<ResultAuthDTO> Handle(LoginCommand request, CancellationToken ct)
        {
            ...
            if (!user.EmailConfirmed)
                throw new UnauthorizedAccessException("E-posta adresiniz henüz onaylanmamış. Lütfen gelen kutunuzu kontrol edin.");

            if (!user.IsApproved)
                throw new UnauthorizedAccessException("Hesabınız henüz bir yönetici tarafından onaylanmamış.");
