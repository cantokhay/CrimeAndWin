...
namespace Action.Application.Mapping
{
    public class GeneralMapping : Profile
    {
        public GeneralMapping()
        {
            ...
            CreateMap<AdminCreateActionDefinitionDTO, ActionDefinition>()
               .ForMember(d => d.Requirements, o => o.MapFrom(s => new ActionRequirements(s.MinPower, s.EnergyCost)))
               .ForMember(d => d.Rewards, o => o.MapFrom(s => new ActionRewards(s.PowerGain, s.ItemDrop, s.MoneyGain)))
               .ReverseMap();

            CreateMap<AdminUpdateActionDefinitionDTO, ActionDefinition>()
                .ForMember(d => d.Requirements, o => o.MapFrom(s => new ActionRequirements(s.MinPower, s.EnergyCost)))
                .ForMember(d => d.Rewards, o => o.MapFrom(s => new ActionRewards(s.PowerGain, s.ItemDrop, s.MoneyGain)))
                .ReverseMap();

            CreateMap<ActionDefinition, AdminResultActionDefinitionDTO>()
                .ForMember(dest => dest.MinPower, opt => opt.MapFrom(src => src.Requirements.MinPower))
                .ForMember(dest => dest.EnergyCost, opt => opt.MapFrom(src => src.Requirements.EnergyCost))
                .ForMember(dest => dest.PowerGain, opt => opt.MapFrom(src => src.Rewards.PowerGain))
                .ForMember(dest => dest.ItemDrop, opt => opt.MapFrom(src => src.Rewards.ItemDrop))
            ...
            // ---- Admin Mappings: PlayerActionAttempt
            CreateMap<AdminCreatePlayerActionAttemptDTO, PlayerActionAttempt>()
                .ForMember(d => d.PlayerActionResults,
                    o => o.MapFrom(s => new PlayerActionResult(
                        s.SuccessRate,
                        s.SuccessRate >= 0.5 ? OutcomeType.Success : OutcomeType.Fail)))
                .ReverseMap();

            CreateMap<AdminUpdatePlayerActionAttemptDTO, PlayerActionAttempt>()
                .ForMember(d => d.PlayerActionResults,
                    o => o.MapFrom(s => new PlayerActionResult(
                        s.SuccessRate,
                        s.SuccessRate >= 0.5 ? OutcomeType.Success : OutcomeType.Fail)))
            ...
            CreateMap<PlayerActionAttempt, AdminResultPlayerActionAttemptDTO>()
                .ForMember(dest => dest.SuccessRate, opt => opt.MapFrom(src => src.PlayerActionResults.SuccessRate))
                .ForMember(dest => dest.OutcomeType, opt => opt.MapFrom(src => src.PlayerActionResults.OutcomeType.ToString()))
                .ReverseMap();
        }
    }
}
